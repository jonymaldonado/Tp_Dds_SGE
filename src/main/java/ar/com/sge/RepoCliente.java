...
/*public Cliente getUsuario(String nombre,String clave)throws IOException{
	List<Cliente> lista = this.daocliente.getAll().stream().filter(s->s.getNombre().equalsIgnoreCase(nombre)&& s.getContraseña().equalsIgnoreCase(clave)).collect(Collectors.toList());
	return lista.get(0);
}*/
